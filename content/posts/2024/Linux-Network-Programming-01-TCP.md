...
- bind
  给套接字赋予一个本地地址，在TCP传输中，服务端必须要bind一个IP地址，因为客户端总是需要向服务端发送请求的，**如果都绑定了地址**，在断连后重连会浪费更多的时间，所以在tcp协议中，并不建议两端都bind，**不bind本地ip地址会报错**，**bind 0.0.0.0 相当于绑定外网地址**，bind的流程
